// Copied from Web VoiceOrb V1; independent H5 lifecycle, no robot overlay.
export class WelcomeOrb {
  ...
  init() {
    ...
    this.canvas = canvas;
    canvas.addEventListener("webglcontextlost", () => this.destroy(), {
      once: true,
    });
  ...
  sync(event) {
    ...
    const running =
      ...
      !this.motion.matches &&
      event?.type !== "pagehide";
    this.container.classList.toggle("is-animated", Boolean(running));
    if (running) {
  ...
  tick(now) {
    if (!this.gl) return;
    this.elapsed += Math.min(now - this.last, 64);
    this.last = now;
    ...
    gl.uniform1f(this.uniforms.time, (this.elapsed / 1000) * 1.4);
    gl.uniform2f(
